...
socketTrace(uint32_t idx, std::string varName, std::string path, auto callback)
{
    ...
    Config::ConnectWithoutContext("/NodeList/" + std::to_string(idx) + 
                                ...
                                MakeBoundCallback(callback, ascii.CreateFileStream(outpath + traffic_config.flows[idx].congestion_control + std::string(" ") + std::to_string(idx+1) + "-" + varName +".csv")));
...
main(int argc, char* argv[])
{
    ...
    for (uint32_t i = 0; i < senders.GetN(); i++) {
        ...
        Simulator::Schedule(Seconds(traffic_config.flows[i].start_time) + MilliSeconds(1), &socketTrace<decltype(&uint32Tracer)>,  senders.Get(i)->GetId(), "bif", "BytesInFlight",  &uint32Tracer);
    ...
    for (uint32_t i = 0; i < senders.GetN(); i++) {
        ...
        rxBytes.push_back(0);
        Simulator::Schedule(Seconds(0.1) + MilliSeconds(1) + Seconds(traffic_config.flows[i].start_time), &TraceThroughput, flowMonitorS, ascii.CreateFileStream(outpath + traffic_config.flows[i].congestion_control + std::to_string(i) + "-throughtput.csv"), i+1, 0, Seconds(0));
        files["throughtput"].push_back(outpath + traffic_config.flows[i].congestion_control + std::to_string(i) + "-throughtput.csv");
        
        Simulator::Schedule(Seconds(0.1) + MilliSeconds(1) + Seconds(traffic_config.flows[i].start_time), &TraceGoodput, ascii.CreateFileStream(outpath + traffic_config.flows[i].congestion_control + std::to_string(i) + "-goodput.csv"), i, 0, Seconds(0));
